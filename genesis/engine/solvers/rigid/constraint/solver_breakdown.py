...
def _kernel_parallel_linesearch_eval(
    ...
    for i_flat in range(_B * _K):
        ...
        if active:
            ...
            # === Phase 3: Cooperative gradient check + bisection + expansion ===
            best_alpha_shared = sh_cand_alpha[0]

            # Phase 3a: Cooperatively compute gradient at best_alpha (or alpha=0 for Branch A)
            # Thread 0 writes the alpha to evaluate; 0.0 means no improvement from grid (Branch A).
            alpha_eval = best_alpha_shared
            if best_alpha_shared <= 0.0:
                alpha_eval = gs.qd_float(0.0)  # Branch A: evaluate grad(0)
            ...
            # Phase 3b: Thread 0 decides action based on gradient
            if tid == 0:
                ...
                g_eval = 2.0 * alpha_eval * qt2_total + qt1_total

                if best_alpha_shared <= 0.0:
                    # --- Branch A: grid found no improvement ---
                    if g_eval < -gtol:
                        # Cost descending at alpha=0 — try expansion via thread-0 sequential eval
                        # Exponential stepping: lo, 4*lo, 16*lo, 64*lo
                        _AMAX = gs.qd_float(qd.static(LS_ALPHA_MAX))
                        exp_a = gs.qd_float(0.0)  # last point with grad < 0
                        exp_alpha = lo
                        for _exp_step in range(4):
                            ...
                            if g_exp > 0.0:
                                ...
                                # Cost improved but still descending — update best and continue
                                constraint_state.candidates[0, i_b] = exp_alpha
                                constraint_state.candidates[4, i_b] = c_exp
                            exp_a = exp_alpha
                            exp_alpha = qd.min(exp_alpha * 4.0, _AMAX)
                            if exp_alpha >= _AMAX:
                                break
                elif qd.abs(g_eval) > gtol:
                    # --- Branch B: grid found improvement, gradient not converged ---
                    if g_eval > 0.0:
                        # Branch B3: overshot — bisect [best/2, best]
                        ...
                        # Branch B2: still descending — exponential expansion from best_alpha
                        _AMAX = gs.qd_float(qd.static(LS_ALPHA_MAX))
                        exp_a = alpha_eval
                        exp_alpha = alpha_eval * 4.0
                        for _exp_step in range(4):
                            if exp_alpha >= _AMAX:
                                break
                            c_exp, g_exp = _ls_eval_cost_grad(exp_alpha, i_b, constraint_state)
                            if g_exp > 0.0:
                                ...
                                if c_mid_b < p0_cost and c_mid_b < constraint_state.candidates[4, i_b]:
                                    ...
                                    constraint_state.candidates[4, i_b] = c_mid_b
                                break
                            elif c_exp < p0_cost and c_exp < constraint_state.candidates[4, i_b]:
                                constraint_state.candidates[0, i_b] = exp_alpha
                                constraint_state.candidates[4, i_b] = c_exp
                            exp_a = exp_alpha
                            exp_alpha = qd.min(exp_alpha * 4.0, _AMAX)
